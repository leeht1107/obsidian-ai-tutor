...
### 대신 AI 에이전트에게 시키기 (VS Code, Antigravity 등)

VS Code의 Copilot Chat(에이전트 모드)이나 Antigravity처럼 터미널 명령을 대신
실행해주는 에이전트형 IDE를 이미 쓰고 있다면, 이 문서 전체와 아래 프롬프트를
그대로 붙여넣어 설치를 통째로 맡겨도 됩니다.

```text
이 문서(README_Ko.md)를 읽고 나 대신 provider CLI 설치를 진행해줘.

- 아직 어떤 provider를 쓸지 안 정했으면, 나한테 물어보고 골라줘.
- 내가 지금 Mac인지 Windows인지는 네가 직접 판단해서 그에 맞는 명령을 써줘.
- 로그인 명령을 빼고는 전부 네가 알아서 실행해. 단계마다 해도 되냐고 나한테
  묻지 말고 바로 진행해.
- 문서에 나온 설치 명령이 안 먹히거나 이미 바뀐 것 같으면, 멈추지 말고 그
  provider의 최신 공식 설치 방법을 웹에서 찾아서 그걸로 대신 설치해줘.
- 로그인 명령(브라우저 인증이 필요한 부분)만 나한테 알려주고 내가 직접 끝낼
  때까지 기다려. 네가 대신 로그인하려고 하지 마.
- 내가 로그인했다고 말하면, 그게 진짜 됐는지는 네가 확인하려 하지 말고
  Obsidian에서 이 플러그인을 열어보라고 알려줘 — 설정 마법사가 실제로
  확인해줄 거야.
- 이 플러그인 자체가 BRAT으로 이미 설치돼 있는데 최신 버전이 아니면, 그것도
  네가 확인해서 업데이트해줘 (Obsidian 명령 팔레트에서
  `BRAT: Check for updates and update all plugins` 실행). 네가 Obsidian
  명령을 직접 실행할 수 없다면, 뭘 눌러야 하는지 나한테 정확히 알려줘.
```

에이전트가 설치·로그인이 됐다고 말해도, 최종 확인은 항상 플러그인 자체의
설정 마법사가 합니다.
